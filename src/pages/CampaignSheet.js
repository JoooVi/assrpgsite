...
import {
  Box, Typography, Paper, CircularProgress, Alert, Grid, Button, TextField,
  List, ListItem, ListItemText, IconButton, Snackbar
} from '@mui/material';
...
const CampaignSheet = () => {
    ...
    const handleSaveNotes = async () => {
        try {
            ...
            console.error("Erro ao salvar as notas:", err.response?.data || err.message);
    ...
    if (loading) return (
        <Box className={styles.loadingContainer}>
            <CircularProgress />
            <Typography variant="h6" sx={{ mt: 2, color: 'white' }}>Carregando campanha...</Typography>
        </Box>
    );
    ...
    if (!campaign) return (
        <Box className={styles.errorContainer}>
            <Alert severity="warning" sx={{ m: 3 }}>Campanha não encontrada ou você não tem permissão para acessá-la.</Alert>
        </Box>
    );

    return (
        <Box className={styles.campaignSheet}>
            <Paper elevation={5} className={styles.mainContent}>
                <Box className={styles.campaignHeader}>
                    <Typography variant="h4" gutterBottom className={styles.campaignNameTitle}>
                    ...
                    <Typography variant="h6" className={styles.masterInfo}>
                        Mestre: {campaign.master?.name || campaign.masterName || "Desconhecido"}
                    </Typography>
                    ...
                    {isMaster ? (
                        <Grid container spacing={3} className={styles.masterShieldGrid}>
                            <Grid item xs={12} md={4} className={styles.shieldColumn}>
                                <Paper elevation={2} className={`${styles.columnPaper} ${styles.playersColumnPaper}`}>
                                    <Typography variant="h5" gutterBottom className={styles.columnTitle}>Status dos Jogadores</Typography>
                                    <Box className={styles.scrollableContent}>
                                    {playersData.length > 0 ? (
                                        <List>
                                            {playersData.map(character => (
                                                <ListItem key={character._id} className={styles.characterListItem}>
                                                    {/* ===== INÍCIO DA CORREÇÃO ===== */}
                                                    <Box sx={{ width: '100%', display: 'flex', flexDirection: 'column' }}>
                                                        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', width: '100%' }}>
                                                            <Typography variant="h6" sx={{color: '#8c1a1a', fontWeight: 'bold', mb: 1 }}>
                                                                {character.name}
                                                            </Typography>
                                                            <IconButton
                                                                component="a"
                                                            ...
                                                            >
                                                                <Typography variant="button" sx={{ fontSize: '0.75rem' }}>Ficha</Typography>
                                                            </IconButton>
                                                        </Box>
                                        ...
                                        <Typography color="text.secondary">Nenhum jogador na campanha.</Typography>
                                    )}
                                    </Box>
                                </Paper>
                            </Grid>
                            ...
                            <Grid item xs={12} md={4} className={styles.shieldColumn}>
                                <Paper elevation={2} className={styles.columnPaper}>
                                    <Typography variant="h5" gutterBottom className={styles.columnTitle}>Rolagem dos jogadores</Typography>
                                    <Box className={styles.scrollableContent}>
                            ...
                            <Grid item xs={12} md={4} className={styles.shieldColumn}>
                                <Paper elevation={2} className={styles.columnPaper} sx={{ marginBottom: '20px' }}>
                                    <Typography variant="h5" gutterBottom className={styles.columnTitle}>Rolar Dados</Typography>
                                    <MasterDiceRoller campaignId={campaignId} />
                                </Paper>

                                <Paper elevation={2} className={styles.columnPaper}>
                                    <Typography variant="h5" gutterBottom className={styles.columnTitle}>Anotações</Typography>
                                    <TextField
